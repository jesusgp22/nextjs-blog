...
function RegisterWithUser(email, password, name, rateLimiting = true) {
  ...
  const ValidatePassword = FqlStatement =>
    If(
      ...
      FqlStatement,
      // Else we Abort!
      Abort('Invalid password, please provide at least 8 characters')
    )
...
function registerWithUser(client, email, password, name) {
  return client
    .query(Call(q.Function('register_with_user'), email, password, name))
